interface Props {
  ...
  setExecShares: (shares: number) => void;
  setExecPrice: (price: number) => void;
  setDefensiveLiquidity: (v: number) => void;
}

const RebalancePanel: React.FC<Props> = (props) => {
  const { taxAwareRebalance, dcaBlocked, dcaAction, monthlyInjection, defensiveLiquidity, availableCash, portfolio, cardStyle, setPendingTrade, setExecShares, setExecPrice, setDefensiveLiquidity } = props;
  if (!taxAwareRebalance || taxAwareRebalance.suggestions.length === 0) return null;
  return (
      {/* REBALANCEO */}
