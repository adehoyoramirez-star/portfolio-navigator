...
function estimateLocalVix(
  ...
  // BUG-06 FIX: usar S&P500/MSCI World como proxy de VIX, NO BTC.
  // BTC vol != miedo sistémico: BTC puede tener vol alta en bull markets (euforia, no pánico).
  // S&P500 / MSCI World reflejan mejor el riesgo sistémico que determina regímenes macro.
  // Preferencia: IS3Q.DE (MSCI World) → VVSM.DE (Semis, proxy risk-on) → BTC como último recurso.
  const spTicker  = backtestTickers['IS3Q.DE'] ?? backtestTickers['VVSM.DE'];
  const btcTicker = backtestTickers['BTC-EUR'] ?? 'BTC-EUR';
  const closes    = (spTicker ? closesHistory[spTicker] : null)
                 ?? closesHistory[btcTicker]
                 ?? [];
  ...
  if (spTicker) {
    // S&P500 vol realizada mapea bien a VIX: vol 12% → VIX ~12; vol 25% → VIX ~25; vol 40% → VIX ~40
    return Math.min(80, Math.max(8, realizedVol));
  } else {
    // BTC vol sobredimensionada: vol 50% en BTC es normal, no es pánico.
    // Factor de escala ~0.55 para calibrar BTC vol a escala VIX histórica.
    return Math.min(60, Math.max(8, realizedVol * 0.55));
  }
}
