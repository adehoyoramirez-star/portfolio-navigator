...
async function fetchYahoo(
  ...
  throw new Error(`No se pudo obtener datos de ${ticker} después de ${retries + 1} intentos: ${errors.join('; ')}`);
}
...
export async function getMacroData(): Promise<MacroData> {
  ...
  try {
    ...
    console.error('Error grave en getMacroData, usando valores por defecto:', error);
    return defaultMacro;
  }
}
