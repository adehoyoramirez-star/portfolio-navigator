...
export interface PersistedMacro {
  ...
  btcDominance?: number;
  mvrvRatio?: number;
  // Monte Carlo Jump Diffusion — AUDIT-FIX-01: separación λ_btc vs λ_portfolio
  jumpIntensity?: number;          // λ_btc: intensidad saltos BTC (modo multivariante Cholesky)
  jumpIntensityPortfolio?: number; // λ_p:   intensidad saltos portfolio completo (fallback univariante)
  jumpMean?: number;               // media log-retorno del salto — debe ser NEGATIVO (crashes)
  jumpStd?: number;
  // BTC Cycle Analyzer
