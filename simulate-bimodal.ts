// simulate-bimodal.ts - Validar comportamiento FIX-BIMODAL
// Run: npx tsx simulate-bimodal.ts
...
function runScenario(label: string, macro: any, returns: number[], btcCycle?: any) {
  ...
  console.log(`KillSwitch: L${result.killSwitchLevel} ${result.killSwitchName}`);
  console.log(`\nAllocations:`);
  result.allocations.forEach(a => {
    console.log(`  ${a.name.padEnd(22)} ${(a.finalAllocation * 100).toFixed(2)}%`);
  });
  if (result.btcCycle) {
    console.log(`BTC Signal: ${result.btcCycle.signal}`);
  }
}

// Scenario 1: EXPANSION - high momentum returns
...
// Scenario 2: CONTRACTION - mixed returns
...
// Scenario 3: CRISIS - negative returns
