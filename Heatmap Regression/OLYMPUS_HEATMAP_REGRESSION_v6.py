...
Uso: python OLYMPUS_HEATMAP_REGRESSION_v6.py [--mode swing|scalp|funda] [--top N] [--no-fred] [--capital CAPITAL_EUR]
...
factor_names = [f"mom{mom_p}", f"rev{rev_p}", f"volratio{vs}_{vl}", f"rs_bench{mom_p}", "rsi2_cs", f"volanom{vanom_p}", f"mom{mom_lp}", "slope"]
...
try:
    ...
    if fred_result is not None:
        ...
        # Solo mantener series frescas para el modelo
        if fresh_cols and len(fresh_cols) < len(fred_result.columns):
            n_pruned = len(fred_result.columns) - len(fresh_cols)
            print(f"   [FRED-PRUNE] {n_pruned} series stale eliminadas")
            fred_result = fred_result[fresh_cols]
    else:
        ...
        if fold_i == 0 and not pruned_factor_indices:
            try:
                ...
                if len(X_tr) > 2000:
                    ...
                    def _obj(trial):
                        p = {
                            'n_estimators': trial.suggest_int('n', 400, 1000),
                            ...
                            'reg_alpha': trial.suggest_float('ra', 0.1, 5.0, log=True),
                            'reg_lambda': trial.suggest_float('rl', 0.1, 5.0, log=True),
                    ...
                    st.optimize(_obj, n_trials=15, show_progress_bar=False)
...
if ic_20d < 0.01:
    icstatus = "🔴"
    print(f"   \n   🚨 ALARMA IC: IC_20d={ic_20d:.4f} < 0.01 — Señal degradada. Revisar modelo.")
elif ic_20d < 0.02:
    icstatus = "🟡"
    print(f"   \n   ⚠️ PRECAUCIÓN IC: IC_20d={ic_20d:.4f} < 0.02 — Señal débil. Monitorear.")
...
if len(q5_tickers) >= 2:
    ...
    # [FIX-I] Pesos: LW covarianza sobre datos PRE-OOS (sin look-ahead)
    # [RISK-PARITY] v7.0: Risk Parity óptimo en lugar de inverso-vol heurístico
    # Minimiza la varianza de las contribuciones al riesgo (RC_i = w_i * (Σw)_i)
    # Target: RC_i = RC_j ∀ i,j | Sujeto a: sum(w)=1, w_i>=0, w_i<=0.25
    ...
    def _risk_parity_objective(w):
        ...
        if port_var <= 0:
            return 1e10
        rc = w * (cov_arr @ w)
        rc_target = port_var / n_a
        return np.sum((rc - rc_target) ** 2)
    
    cons = ({'type': 'eq', 'fun': lambda w: np.sum(w) - 1.0})
    bounds = [(0.0, 0.25)] * n_a
    w0 = np.ones(n_a) / n_a
    result = minimize(_risk_parity_objective, w0, method='SLSQP', bounds=bounds,
                      constraints=cons, options={'maxiter': 1000, 'ftol': 1e-12})
    if result.success:
        w_final = result.x
        print(f"   [RISK-PARITY] Pesos óptimos encontrados | HHI={np.sum(w_final**2):.4f} | iter={result.nit}")
    else:
        vol_q5  = np.sqrt(np.diag(cov_arr))
        inv_vol = 1.0 / np.maximum(vol_q5, 1e-8)
        raw_w   = inv_vol / inv_vol.sum()
        w_cap   = np.minimum(raw_w, 0.25)
        w_final = w_cap / w_cap.sum()
        print(f"   ⚠️ [RISK-PARITY] No convergió ({result.message}), usando inverso-vol")
    ...
    # [GREEDY-SHARPE] Seleccion greedy: orden optimo de entrada segun Sharpe marginal
    # Usa covarianza LW + raw_pred para maximizar Sharpe del portfolio en cada paso
    greedy_order = []
    try:
        n_greedy = len(q5_tickers_complete)
        if n_greedy >= 2:
            mu_s = pd.Series({t: results.get(t, {}).get("raw_pred", 0) for t in q5_tickers_complete})
            mu_std = float(mu_s.std()) + 1e-10
            mu_arr = (mu_s / mu_std * 0.10).values
            cov_ann = cov_q5.values * 252
            n_tot = len(q5_tickers_complete)
            sel = []
            rem = list(range(n_tot))
            for _ in range(n_tot):
                best_s = -1e10
                best_i = -1
                for idx in rem:
                    test = sel + [idx]
                    sc = cov_ann[np.ix_(test, test)]
                    sv = np.sqrt(np.diag(sc))
                    iv = 1.0 / np.maximum(sv, 1e-8)
                    w = iv / iv.sum()
                    pr = w @ mu_arr[test]
                    pv = w @ sc @ w
                    sh = pr / np.sqrt(pv) if pv > 0 else -1e10
                    if sh > best_s:
                        best_s = sh
                        best_i = idx
                if best_i >= 0:
                    sel.append(best_i)
                    rem.remove(best_i)
                    greedy_order.append(q5_tickers_complete[best_i])
        print(f"   [GREEDY-SHARPE] Orden optimo de {len(greedy_order)} tickers | 1er={greedy_order[0] if greedy_order else 'N/A'}")
    except Exception as e:
        print(f"   [GREEDY-SHARPE] Error ({e}), usando fallback por confianza")
        greedy_order = []
...
if portfolio_weights:
    ...
    # [GREEDY-SHARPE] Usar orden optimo de entrada segun Sharpe marginal
    if greedy_order:
...
<div class="footer">
  ...
  <code style="color:#fff">python OLYMPUS_HEATMAP_REGRESSION_v6.py --mode {MODE_NAME}{" --top "+str(TOP_N) if TOP_N else ""}{" --no-fred" if NO_FRED else ""}{" --capital "+str(CAPITAL_EUR) if CAPITAL_EUR!=10000 else ""}</code>
