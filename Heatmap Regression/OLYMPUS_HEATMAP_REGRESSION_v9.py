#!/usr/bin/env python3
'''
OLYMPUS HEATMAP REGRESSION v9.0 - FACTOR LABORATORY
====================================================
...
parser = argparse.ArgumentParser(description='OLYMPUS HEATMAP REGRESSION v9.0 Factor Lab')
...
print('OLYMPUS HEATMAP REGRESSION v9.0 - FACTOR LABORATORY')
print(f'~25 factores | IC individual | Composite con IC > {MIN_IC_FOR_FACTOR} | FINRA incluido')
print('=' * 70)
...
for ti, t in enumerate(available):
    ...
    all_ind[t] = {'ret': ret, 'log_r': log_r, 'cumlog': cumlog, 'c': c,
                  ...
                  'rsi2': r2, 'rsi14': r14, 'atr_pct': atr_pct,
...
print(f'\nFactores con IC > {MIN_IC_FOR_FACTOR}: {len(selected)}')
if selected:
...
# --- Build composite (nanmean para tolerar NaN en factores individuales) ---
print(f'\nComposite: {len(selected_names)} factores')
composite = None
for name in selected_names:
    df = factor_catalog[name]['df']
    composite = df.copy() if composite is None else composite + df
composite = composite / len(selected_names) if len(selected_names) > 0 else cs_rank(ret_df * 0) + 50
# Reemplazar NaN con 50 (neutral) en el composite final
composite = composite.fillna(50.0).clip(0, 100)

# --- Walk-Forward OOS IC ---
print(f'\nWalk-Forward OOS IC: {N_FOLDS} folds | H={HORIZON}d')
...
if valid_today.sum() < 10:
    today_comp = np.nan_to_num(comp_np[last_day], nan=50.0)
...
if portfolio_weights:
    ...
    for t, w in sorted_items:
        ...
        atr_raw = all_ind[t]['atr_pct'] if t in all_ind else np.array([0.02])
        atr = float(atr_raw[~np.isnan(atr_raw)][-1]) if len(atr_raw[~np.isnan(atr_raw)]) > 0 else 0.02
...
<title>OLYMPUS v9.0 FACTOR LAB</title><style>
...
<h1>OLYMPUS <span>FACTOR LABORATORY v9.0</span></h1>
<div class=sub>{len(available)} tickers | {LOOKBACK}d | {dt} | {len(factor_catalog)} factores testeados | {len(selected_names)} seleccionados | FINRA incluido</div>
<div class=banner>
...
html += f'<div class=footer>OLYMPUS v9.0 FACTOR LAB | {len(factor_catalog)} factores testeados | {len(selected_names)} seleccionados | IC={mean_ic:.4f} | IR={ir:.2f} | {len(all_ic_scores)}d OOS</div></body></html>'
...
print()
print('=' * 70)
print(f'  RESUMEN OLYMPUS v9.0 FACTOR LABORATORY')
print(f'  Factores testeados: {len(factor_catalog)} | Seleccionados (|IC| > {MIN_IC_FOR_FACTOR}): {len(selected_names)}')
...
if selected: print(f'  Mejores factores: {", ".join([n for n, r in selected[:5]])}')
if fold_ic_means:
...
print(f'  Outputs: predictions.csv | portfolio_q5.csv | factor_ic_report.csv | heatmap_dashboard.html')
print('=' * 70)
