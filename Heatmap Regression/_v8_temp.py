#!/usr/bin/env python3
'''
OLYMPUS HEATMAP REGRESSION v8.0 - Hende Fund - SIMPLE EDITION
=============================================================
7 factores tecnicos -> ranking cross-seccional -> media equiponderada
Sin ML, sin hyperopt, sin datos alternativos. Solo matematica simple.

Uso: python OLYMPUS_HEATMAP_REGRESSION_v8.py [--capital CAPITAL_EUR]
Output: heatmap_dashboard.html, predictions.csv, portfolio_q5.csv, ibkr_orders.csv
'''
...
parser = argparse.ArgumentParser(description='OLYMPUS HEATMAP REGRESSION v8.0')
parser.add_argument('--capital', type=float, default=10000.0, help='Capital EUR')
parser.add_argument('--max-positions', type=int, default=0, help='Max posiciones')
...
CACHE_MAX_AGE = 24 * 3600; # MIN_DAILY_VOLUME removido: filtro de liquidez desactivado (volume_df del cache tiene datos irregulares)
...
print('OLYMPUS HEATMAP REGRESSION v8.0 - SIMPLE EDITION')
print('7 factores | Ranking cross-seccional | Sin ML | Sin alt data')
print('=' * 70)
...
# El volume_df del cache tiene datos irregulares (NaN en la mayoria de tickers).
# Usamos todos los tickers del cache, que ya estan filtrados por >=90% de datos de precio.
print(f'Filtro de liquidez DESACTIVADO - usando todos los {len(available)} tickers del cache')
# Para el heatmap, los que no tengan volumen apareceran con vol=0 (ultimos en el ranking visual)
...
# --- Indicadores (minimos para 7 factores) ---
print('Calculando indicadores minimos...')
all_ind = {}
for t in available:
    ...
    all_ind[t] = {'returns': ret, 'close': c, 'ma20': ma20, 'ma50': ma50, 'rsi2': r2, 'atr_pct': atr_pct}
print(f'   OK {len(all_ind)} tickers con indicadores')
...
# --- 7 FACTORES (los que sobreviven la poda en v7) ---
print('Construyendo 7 factores cross-seccionales...')

mom21 = np.expm1(log_ret.rolling(21).sum())
rev5 = -np.expm1(log_ret.rolling(5).sum())
vol5 = ret_df.rolling(5).std(); vol63 = ret_df.rolling(63).std().replace(0, np.nan)
volratio = -(vol5 / vol63)
bench_median = mom21.median(axis=1).fillna(0.0)
rs_bench21 = mom21.subtract(bench_median, axis=0)
...
factor_list = [mom21, rev5, volratio, rs_bench21, rsi2_cs, mom63, -atr_df]
factor_names = ['mom21', 'rev5', 'volratio5_63', 'rs_bench21', 'rsi2_cs', 'mom63', 'atr_pct_low']

# --- Ranking CS ---
def cs_rank(df):
    return df[tickers_panel].rank(axis=1, pct=True) * 100

ranked = [cs_rank(f) for f in factor_list]
composite = sum(ranked) / len(ranked)
print(f'   Composite = media de {len(factor_list)} rankings CS')
...
# --- Walk-Forward OOS IC ---
print(f'Walk-Forward OOS IC: {N_FOLDS} folds | H={HORIZON}d | LOOKBACK={LOOKBACK}d')

n_days = len(ret_df); START_DAY = 91; TRAIN_END = n_days - HORIZON - 5
...
comp_np = composite[tickers_panel].values
fwd_np = fwd_excess[tickers_panel].values
all_ic_scores = []; fold_ic_means = []
...
for fold_i in range(N_FOLDS):
    ...
    for day_i in range(te_start, te_end):
        comp_day = comp_np[day_i]; fwd_day = fwd_np[day_i]
    ...
    print(f'   Fold {fold_i+1}/{N_FOLDS}: train=[{START_DAY},{tr_end}) test=[{te_start},{te_end}) | IC={fold_ic:.4f} ({len(fold_ics)}d OOS)')
...
# --- Matriz de correlaciones LW ---
print('Matriz de correlaciones Ledoit-Wolf...')
complete_tickers = [t for t in all_ind if np.isnan(all_ind[t]['returns']).sum() == 0]
...
print(f'   LW: {len(complete_tickers)} tickers completos')
...
if len(q5_tickers) >= 2:
    q5_complete = [t for t in q5_tickers if np.isnan(all_ind[t]['returns']).sum() == 0]
    if len(q5_complete) < 2: q5_complete = q5_tickers
    q5_ret = pd.DataFrame({t: all_ind[t]['returns'] for t in q5_complete})
    ...
    bh_sharpe = 0.0
    if 'SPY' in all_ind:
        spy_ret_oos = all_ind['SPY']['returns'][last_fold_start:last_fold_end]
...
if portfolio_weights:
    ...
    for t, w in sorted_items:
        ...
        if entry <= 0: continue
        atr = float(all_ind[t]['atr_pct'][-1]) * entry if t in all_ind else entry * 0.02
        sl_dist = max(atr * 2.5, entry * 0.01)
...
if ibkr_rows:
    pd.DataFrame(ibkr_rows).to_csv(os.path.join(BASE_DIR,'ibkr_orders.csv'), index=False)
    print(f'   IBKR ordenes guardadas')

if not corr_hm.empty:
    corr_hm.to_csv(os.path.join(BASE_DIR,'correlations.csv'))

ic_row = pd.DataFrame([{'date':datetime.now().strftime('%Y-%m-%d %H:%M'),'ic_mean':mean_ic,'ic_20d':ic_20d,'ic_60d':ic_60d,'ic_120d':ic_120d,'ir':ir,'hit_rate':hit_rate,'n_days':len(ic_series),'mode':'v8_simple','n_tickers':len(results)}])
...
<title>OLYMPUS v8.0 SIMPLE</title><style>
...
<h1>OLYMPUS <span>HEATMAP REGRESSION v8.0</span> - SIMPLE EDITION</h1>
<div class=sub>{len(available)} activos liquidos | {LOOKBACK}d | {dt} | 7 factores | Ranking CS | Sin ML</div>
<div class=banner>
...
html += f'<div class=footer>OLYMPUS v8.0 SIMPLE | 7 factores CS | Sin ML | Sin hyperopt | IC={mean_ic:.4f} | IR={ir:.2f} | {len(all_ic_scores)}d OOS</div></body></html>'
...
try:
    ...
    q5_export = {'generatedAt':datetime.now().isoformat(),'mode':'v8_simple',
        'modelMetrics':{'ic':mean_ic,'ir':ir,'hitRate':hit_rate},
...
print()
print('=' * 70)
print(f'  RESUMEN OLYMPUS v8.0 SIMPLE')
print(f'  Activos: {len(available)} liquidos (>10M USD/dia) | {len(all_ic_scores)}d OOS | H={HORIZON}d | {N_FOLDS} folds')
print(f'  IC={mean_ic:.4f} | IC_20d={ic_20d:.4f} | IR={ir:.2f} | Hit={hit_rate:.1%} | Semaforo: {icstatus}')
if fold_ic_means:
    print(f'  IC por fold: ' + ' | '.join(f'F{i+1}={fic:.4f}' for i,fic in enumerate(fold_ic_means)))
if portfolio_metrics:
    pm2 = portfolio_metrics
    print(f'  Q5 Portfolio OOS ({pm2["oos_days"]}d): Sharpe={pm2["sharpe"]:.2f} | Net={pm2["sharpe_net"]:.2f} | EW={pm2["ew_sharpe"]:.2f} | SPY={pm2["bh_sharpe"]:.2f}')
    print(f'  Alpha vs SPY: {pm2["sharpe"]-pm2["bh_sharpe"]:+.2f} | MaxDD={pm2["max_dd"]:.1f}%')
print(f'  Outputs: predictions.csv | portfolio_q5.csv | ibkr_orders.csv | heatmap_dashboard.html')
print('=' * 70)
