// supabase/functions/telegram-alerts/index.ts
// FIXES aplicados:
//   - geminiNarrative → aiNarrative (alineado con dashboard Llama)
//   - Label "🤖 Gemini:" → "🦙 Llama (Olympus AI):"
//   - Añadido campo aiModel en daily_summary para mostrar el modelo usado
...
interface TelegramBody {
  ...
  // ACTUALIZADO: antes "geminiNarrative" — ahora "aiNarrative" (motor Llama local)
  aiNarrative?:      string;
  aiModel?:          string;   // e.g. "llama3.1:8b"
  // Compatibilidad hacia atrás (llamadas antiguas)
...
function buildTailRisk(body: TelegramBody): string {
  return [
    ...
    "",
    `→ No realizar compras adicionales hasta que el overlay se desactive.`,
    `⏰ ${new Date().toLocaleString("es-ES", { timeZone: "Europe/Madrid" })}`,
  ].join("\n");
}
...
function buildBlackSwan(body: TelegramBody): string {
  return [
    ...
    "",
    `<b>Análisis Olympus AI:</b> ${body.blackSwanReason ?? "N/D"}`,
    `<b>Régimen:</b> ${body.currentRegime ?? "N/D"}`,
    `<b>VIX:</b> ${body.vix?.toFixed(1) ?? "N/D"}`,
    "",
    `⚡ Revisar coberturas y liquidez <b>inmediatamente</b>.`,
    `⏰ ${new Date().toLocaleString("es-ES", { timeZone: "Europe/Madrid" })}`,
  ].join("\n");
}
...
function buildDailySummary(body: TelegramBody): string {
  ...
  // Compatibilidad: aceptar tanto aiNarrative (nuevo) como geminiNarrative (legado)
  const narrative = body.aiNarrative ?? body.geminiNarrative ?? null;
  const modelLabel = body.aiModel ? `🦙 ${body.aiModel}` : "🦙 Olympus AI";
  ...
  return [
    ...
    "",
    narrative
      ? `${modelLabel}: ${narrative.slice(0, 300)}`
      : "",
    "",
    `⏰ ${new Date().toLocaleString("es-ES", { timeZone: "Europe/Madrid" })}`,
  ]
